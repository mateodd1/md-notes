...
<main class="versions-shell">
    ...
    @if ($versions->isEmpty())
        <section class="empty-versions"><h2>Aún no hay versiones</h2><p>Las versiones se guardan al crear o modificar la nota. Se conservan hasta 50 por nota y durante 7 días.</p></section>
    @else
        <section class="version-list" aria-label="Versiones guardadas">
            @foreach ($versions as $version)
                <article class="version-card"><div><h2>{{ $version->created_at->format('d/m/Y') }} · {{ $version->created_at->format('H:i:s') }}</h2><p>{{ mb_strlen($version->content) }} caracteres · guardada {{ $version->created_at->diffForHumans() }}</p></div><div class="version-actions"><a class="button secondary small" href="{{ route('versions.show', ['version' => $version]) }}">Ver</a><a class="button secondary small" href="{{ route('versions.download', ['version' => $version]) }}">Descargar</a><a class="button small" href="#restore-version-{{ $version->id }}">Restaurar</a></div></article>
