...
<main class="auth-shell">
    <section class="auth-card">
        ...
        <form method="post" action="{{ route('password.update') }}" autocomplete="off">
            ...
            <label for="email">{{ __('ui.email') }}</label><input id="email" name="email" type="email" value="{{ is_string(old('email', $email)) ? old('email', $email) : '' }}" autocomplete="off" autocapitalize="none" spellcheck="false" required autofocus>
