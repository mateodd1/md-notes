...
Route::domain($separateWorkspace ? $publicHost : null)->group(function (): void {
...
Route::domain($separateWorkspace ? $workspaceHost : null)->group(function () use ($separateWorkspace): void {
    ...
    Route::get('/share/{token}/media/{filename}', [ShareController::class, 'media'])
        ...
        ->where('filename', '[a-z0-9]{24}\.[a-z0-9]{1,10}')
        ->middleware('throttle:60,1,shares.media:')
        ->name('shares.media');
    Route::post('/share/{token}/copy', [ShareController::class, 'copyToSpace'])
        ->where('token', '(?:[0123456789ABCDEFGHJKLMNPQRSTUVWXYZ]{5}|[0123456789ABCDEFGHJKLMNPQRSTUVWXYZ]{7})')
        ->middleware('auth')
        ->name('shares.copy');
    Route::get('/share/{token}', [ShareController::class, 'show'])->where('token', '(?:[0123456789ABCDEFGHJKLMNPQRSTUVWXYZ]{5}|[0123456789ABCDEFGHJKLMNPQRSTUVWXYZ]{7})')->middleware('throttle:60,1,shares.show:')->name('shares.show');

    Route::middleware('guest')->group(function (): void {
        Route::get('/login', [AuthController::class, 'create'])->name('login');
        Route::post('/login', [AuthController::class, 'store'])->middleware('throttle:6,1,login.store:')->name('login.store');
        ...
        Route::post('/forgot-password', [AuthController::class, 'sendResetLink'])->middleware('throttle:3,1,password.email:')->name('password.email');
        Route::get('/reset-password/{token}', [AuthController::class, 'resetPasswordForm'])->name('password.reset');
        Route::post('/reset-password', [AuthController::class, 'resetPassword'])->middleware('throttle:3,1,password.update:')->name('password.update');
    });
