(() => {
    ...
    function showToast(message, type = '') { const element = getToast(); clearTimeout(toastTimer); element.className = `toast ${type}`; element.setAttribute('role', type === 'error' ? 'alert' : 'status'); element.setAttribute('aria-live', type === 'error' ? 'assertive' : 'polite'); element.replaceChildren(); const label = document.createElement('span'); label.textContent = message; const close = document.createElement('button'); close.type = 'button'; close.setAttribute('aria-label', t.close); close.innerHTML = svgIcon('close', 'icon close-icon'); close.onclick = () => dismissToast(element); element.append(label, close); animateToast(element); toastTimer = setTimeout(() => dismissToast(element), 3000); }
    ...
    document.addEventListener('click', (event) => { document.querySelectorAll('.profile-menu[open]').forEach((menu) => { if (!menu.contains(event.target)) menu.removeAttribute('open'); }); });
    ...
    const clearLongPress = () => { if (longPressTimer) clearTimeout(longPressTimer); longPressTimer = null; longPressOrigin = null; };
    tree.addEventListener('pointerdown', (event) => { if (event.pointerType !== 'touch' || !event.isPrimary) return; const item = event.target.closest('[data-context-path]'); longPressTriggered = false; longPressOrigin = { item, x: event.clientX, y: event.clientY }; longPressTimer = setTimeout(() => { if (!longPressOrigin) return; longPressTriggered = true; suppressTreeClickUntil = Date.now() + 800; openContextMenu(longPressOrigin.item, longPressOrigin.x, longPressOrigin.y); navigator.vibrate?.(10); }, 500); });
    ...
    const showHistoryVersion = async (id) => { historyLoading(); try { const response = await fetch(versionUrl(id), { headers: { Accept: 'application/json' } }); const version = await response.json(); if (!response.ok) throw new Error(version.message || t.couldNotOpenVersion); historyContent.replaceChildren(); const back = document.createElement('button'); back.type = 'button'; back.className = 'button secondary small history-back'; back.innerHTML = svgIcon('arrowLeft', 'icon button-icon'); back.append(document.createTextNode(t.allVersions)); back.addEventListener('click', () => openVersionHistory(historyPath.textContent)); const title = document.createElement('h3'); title.textContent = version.title; const meta = document.createElement('p'); meta.textContent = `${version.path} · ${version.saved_at}`; const preview = document.createElement('article'); preview.className = 'history-preview markdown-body'; preview.innerHTML = version.rendered; historyContent.append(back, title, meta, preview); } catch (error) { historyContent.replaceChildren(); const message = document.createElement('p'); message.textContent = error.message || t.couldNotOpenVersion; historyContent.append(message); } };
    ...
    const updateActiveNoteLocation = () => {
        if (!activePath) return;
        const url = noteUrl(activePath);
        window.history.replaceState({}, '', url);
        displayedPathname = new URL(url, window.location.href).pathname;
        const noteForm = document.getElementById('note-form');
        if (noteForm) noteForm.action = url;
        const deleteForm = document.querySelector('#confirm-delete form');
        if (deleteForm) deleteForm.action = url;
        const subtitle = document.querySelector('.toolbar-sub');
        if (subtitle) subtitle.textContent = activePath;
    };
    const mutateTree = async (url, fields, successMessage) => {
        ...
        } catch (error) { showToast(error.message || t.couldNotMove, 'error'); }
        finally { if (previousLayout) previousLayout.inert = false; navigating = false; }
    };
    const moveItem = (source, destination) => { if (source && source !== destination) return mutateTree(config.urls.move, { source, destination }, t.itemMoved); };
    const reorderNote = (source, target, position) => { if (source && target && source !== target) return mutateTree(config.urls.reorderNotes, { source, target, position }, t.noteReordered); };
    const reorderFolder = (source, target, position) => { if (source && target && source !== target) return mutateTree(config.urls.reorderFolders, { source, target, position }, t.folderReordered); };
    ...
    let editor = null; let state = null; let editorLayout = null; let editNoteButton = null; let readNoteButton = null; let saveNoteButton = null; let saver = null; let lastSaveError = ''; let uploadingImage = false; let navigating = false;
    const updateSaveState = (status = '') => {
        if (!state || !editorLayout) return;
        if (editorLayout.classList.contains('is-reading')) { state.hidden = true; return; }
    ...
    const bindEditor = async () => {
        ...
        saver = new window.MdNotesSaver(() => boundEditor.value, async (content, snapshot) => {
            ...
            if (!response.ok) {
                const result = await response.json().catch(() => ({}));
                throw new Error(result.errors?.content?.[0] || result.message || t.couldNotSave);
            }
            if (!(await response.json())?.savedAt) throw new Error(t.couldNotSave);
        }, editor.value, (status, error) => {
    ...
    const navigateToNote = async (url, pushHistory = true) => {
        ...
        if (targetUrl.pathname === displayedPathname) return;
        navigating = true;
        const previousLayout = editorLayout;
        if (previousLayout) previousLayout.inert = true;
        try {
            ...
            const nextTree = nextDocument.getElementById('tree');
            if (!nextContent || !nextTree) throw new Error(t.couldNotOpenNote);
            document.querySelector('.content').replaceWith(nextContent);
            tree.innerHTML = nextTree.innerHTML;
            replaceDeleteModal(nextDocument);
            ...
            if (previousLayout) previousLayout.inert = false;
            navigating = false;
        }
    };
