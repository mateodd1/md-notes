...
class RegistrationTest extends TestCase
{
    ...
    public function test_first_registered_account_is_admin_and_gets_the_legacy_import(): void
    {
        Mail::fake();
        ...
        $this->assertDatabaseHas('users', ['email' => 'mateo@example.test', 'is_admin' => true]);
    ...
    public function test_demo_profile_settings_are_disabled_server_side(): void
    {
        ...
        $this->actingAs($demo)->get(route('profile.edit'))
            ->assertRedirect(route('notes.index'))
            ->assertSessionHas('status', 'La configuración está desactivada en la cuenta demo.');
    }
